...
class MySQLDatabase(DatabaseInterface):
    def __init__(self, host : str, user : str, password : str, database : str) -> None:
        ...
        try:
            ...
            Log.info(f"Succesfully connected to MySQL Database. MySQL DB version is {cursor.fetchone()[0]}")
        except mysql.Error as e:
            Log.exception("Failed to connect to MySQL database. Check if it is running", e)
            raise Exception(f"Failed to connect to MySQL database. Check if it is running. Error: {e}")
    def __del__(self):
        try:
