...
class Authentication:
    ...
    def _match(password: str) -> bool:
        ...
        if not Authentication.is_available():
            raise FileNotFoundError(
                f"Password file {Authentication.AUTH_FILE} not found!"
            )
    ...
    def _validate_password(password: str, out_function, in_function) -> bool:
        ...
        if len(password) >= 8:
            out_function(
                Fore.GREEN
                + "Test #1. Password length is greater than or equal to eight"
                + Style.RESET_ALL
            )
        else:
            raise Authentication.TestError(
                Fore.RED
        ...
        if any(char.isupper() for char in password):
            out_function(
                Fore.GREEN
                + "Test #2. The password contains at least one uppercase character"
                + Style.RESET_ALL
            )
        else:
            raise Authentication.TestError(
        ...
        if any(char.isdigit() for char in password):
            out_function(
                Fore.GREEN
                + "Test #3. The password contains at least one digit."
                + Style.RESET_ALL
            )
        else:
            raise Authentication.TestError(
    ...
    def register(out_function, in_function) -> None:
        while True:
            password = in_function(
                "Enter your password to log in to the admin panel later: "
            )
            ...
            try:
                ...
                if confirm_password == password:
                    ...
                    out_function(
                        Fore.GREEN
                        + "The password has been successfully confirmed and set"
                        + Style.RESET_ALL
                    )
                    return
                else:
