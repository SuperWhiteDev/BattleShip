...
from enums import UserConnectionStatus, GameDataCode, GameDataType, Errors, ErrorMessages
from ui import UI
...
class BattleShip:
    ...
    def play_game(self):
        ...
        server_ip, server_port = self.ui.get_server()
    ...
    def handle_error(self, error : dict) -> None:
        ...
        if "error_code" in error:
            if error["error_code"] == Errors.UNEXPECTED_PACKET.value:
                error_code = "<Unexpected packet get sended>:"
            if error["error_code"] == Errors.UNCORRECT_PACKET.value:
                error_code = "<Uncorrect packet ged sended>:"
        if "msg" in error:
            if error["msg"] == ErrorMessages.PLAYER_NOT_IN_ANY_SESSION.value:
    ...
    def server_handle_user_connection_status(
        self, status: UserConnectionStatus
    ):
        # print(f"Status is {status}")
        if status == UserConnectionStatus.BANNED.value:
            print("You have been banned on this server.")
        if status == UserConnectionStatus.DISCONNECTED.value:
            print("You has been disconnected from the server.")
        elif status == UserConnectionStatus.REGISTER_REQUIRED.value:
            print(
            ...
            while True:
                ...
                if response.code == Packet.Code.OK:
                    ...
                    return None
        elif status == UserConnectionStatus.AUTHORIZATION_REQUIRED.value:
            while True:
    ...
    def connect_to_session(self):
        self.connection.delay_send(Packet(Packet.Code.STATUS, UserConnectionStatus.FIND_NEW_SESSION.value))

    def leave_session(self):
        self.connection.delay_send(Packet(Packet.Code.STATUS, UserConnectionStatus.LEAVE_SESSION.value))
    ...
    def handle_session_connection(self, data: dict):
        if "code" in data:
            ...
            if code == GameDataCode.SESSION_STARTED.value:
                ...
                #print("Your ships placement has been successfully confirmed on the server.")
                pass
            ...
            return Packet(Packet.Code.ERROR, {"error_code": Errors.UNCORRECT_PACKET.value})
